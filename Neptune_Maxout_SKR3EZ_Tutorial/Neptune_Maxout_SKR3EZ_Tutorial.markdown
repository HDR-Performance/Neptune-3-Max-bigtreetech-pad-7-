...
> **📌 Note**  
> All Neptune 3 series harness connectors are **plug-and-play** with the SKR 3 EZ—no rewiring needed! The TMC5160 drivers use SPI mode configured via software in `printer.cfg`, so no physical jumpers are required. The display is either the BTT Pad 7 or web-based Mainsail (Raspberry Pi 4/5).
...
2. **Verify Connections**:
   ...
   - Test LEDs: `SET_LED LED=LED_Light WHITE=0.25` (25% brightness, if enabled).
...
1. **Copy Additional Files**: Place `KAMP_Settings.cfg`, `mainsail.cfg`, `timelapse.cfg`, `Line_Purge.cfg`, `Smart_Park.cfg`, and `Adaptive_Meshing.cfg` in your Klipper config directory. Find them in this repository.
2. **Test a Print**: Run the `START_PRINT` macro to test all components.
3. **Save Config**: After PID tuning, run `SAVE_CONFIG` to store settings.
